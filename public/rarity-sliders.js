export class RaritySliders extends HTMLElement {
  ...
  connectedCallback() {
    this._isCollapsed = window.innerWidth <= 600;
    this.updateVisibility();
  ...
  render() {
    ...
    this.rarityList.forEach((item, index) => {
      ...
      slider.addEventListener("input", (e) => {
        ...
        if (total > 100) {
          let diff = total - 100;
          this.rarityList[index].prob = Math.max(newVal - diff, 0);
          slider.value = this.rarityList[index].prob;
          span.textContent = `${this.rarityList[index].prob}%`;
        }
  ...
  dispatchRarityUpdated() {
    ...
    this.dispatchEvent(new CustomEvent("rarity-updated", { detail, bubbles: true, composed: true }));
  }
}
